...
class WOFComputerPlayer(WOFPlayer): 
    ...
    def getPossibleLetters(self, guessed): 
        possible_letters = []
        for i in [letter for letter in WOFComputerPlayer.SORTED_FREQUENCIES]: 
                if i not in guessed:
                    if self.prizeMoney >= WOFComputerPlayer.VOWEL_COST: 
                        possible_letters.append(i)
                    elif i not in WOFComputerPlayer.VOWELS: 
                        possible_letters.append(i)
    ...
    def getMove(self, category, obscuredPhrase, guessed): 
        ...
        if len(possible_letters) == 0: move = 'pass'
        else: 
            if self.smartCoinFlip(): 
                move = possible_letters[len(possible_letters)-1]
            else: 
                move = random.choice(possible_letters) # [letter for letter in possible_letters]
        return move      
